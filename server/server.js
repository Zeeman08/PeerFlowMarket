...
app.post("/createStore", async (req, res) => {
  try{
    ...
    const results = await db.query(
      "INSERT INTO storefront (description, rating, last_updated_on, image) VALUES ($1, $2, $3) RETURNING *",
      [req.body.description, req.body.rating, req.body.last_updated_on, req.body.image]
    );
    res.status(201).json({
      status: "success",
      results: results.rows.length,
